...
##
## Choosing who to run is all it does now. It used to double as the capture
## prompt, picking a species out of a catalogue after a boss; captures happen in
## the fight itself now, against something actually standing in front of you.
...
func _refresh() -> void:
	...
	var matches: Array = PokeCharacters.search(query, ROWS)
	_rows = matches
